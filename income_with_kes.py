            ...
            if date == self.first_armotization["date"]:
                if self.face_value <= 1_000_000:
                    interest_list.append({"date": date, 
                                          "interest": f'Kes {currencify(interest)}', 
                                          "outstanding principal": f'Kes {currencify(0)}', 
                                          "amortization_payout": f'Kes {0}',
                                          "final redemption": f'Kes {currencify(self.face_value+interest)}'})
                    break
                else:
                    pr1 = self.first_armotization["proportion"]
                    amortization_payout = pr1*self.face_value
                    amortization_payouts += amortization_payout
                    interest_list.append({"date": date, "outstanding principal": f'Kes {currencify(self.face_value)}', 
                                          "interest": f'Kes {currencify(interest)}', 
                                          "outstanding principal": f'Kes {currencify(self.face_value)}',
                                          "amortization_payout": f'Kes {currencify(amortization_payout)}',
                                          "total payout": f'Kes {currencify(amortization_payout + interest)}'})
                ...
                interest_list.append({"date": date, "outstanding principal": f'Kes {currencify(self.face_value)}', 
                                      "interest": f'Kes {currencify(interest)}', 
                                      "outstanding principal": f'Kes {currencify(self.face_value)}',
                                      "amortization payout": f'Kes {currencify(amortization_payout)}',
                                      "total payout": f'Kes {currencify(amortization_payout + interest)}'})
                self.face_value = (1-pr2) * self.face_value
            else:
                if date != IFB_Feb2024.PAYMENT_DATES[-1]:
                    interest_list.append({"date": date, "outstanding principal": f'Kes {currencify(self.face_value)}', "interest": f'Kes {currencify(interest)}'})
                else:
                    total_payout = self.face_value + sumup_interest + amortization_payouts
                    interest_list.append({"date": date,
                                          "interest":  f'Kes {currencify(interest)}',
                                          "outstanding principal": f'Kes {currencify(self.face_value)}', 
                                          "total interest payments": f'Kes {currencify(sumup_interest)}',
                                          "final redemption": f'Kes {currencify(self.face_value+interest)}',
                                          "total amortization payout": f'Kes {currencify(amortization_payouts)}',
                                          "total amount paid back since investment": f'Kes {currencify(total_payout)}'})
